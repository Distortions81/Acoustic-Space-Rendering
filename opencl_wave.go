...
//go:embed opencl_wave_kernel.cl
var waveKernelSource string
...
func newOpenCLWaveSolver(width, height int) (*openCLWaveSolver, error) {
	...
	if err := solver.kernel.SetArgs(
		int32(width),
		int32(height),
		solver.realKernelArg(waveDamp32),
		solver.realKernelArg(waveSpeed32),
		solver.realKernelArg(float32(2.0)),
		solver.realKernelArg(float32(4.0)),
		solver.currBuf,
		solver.prevBuf,
		solver.nextBuf,
		solver.wallMaskBuf,
...
func (s *openCLWaveSolver) realKernelArg(value float32) interface{} {
	if s.useFP16 {
